function updateCoords(n) {
    jQuery("#x").val(n.x), jQuery("#y").val(n.y), jQuery("#w").val(n.w), jQuery("#h").val(n.h)
}
var croper;
$(function () {
    new Uploader(
        ...
        function (n) {
            ...
            $("#imgsrc").val(n), croper ? croper.setImage(n) : croper = new Croper($("#crop_image"), new Previewer([[$("#preview_large"), 100]]));
            //$("#imgsrc").css("width", 400);
    ...
    $("#form_crop").ajaxForm({
        success: function (n) {
            $("#crop_image").attr("src", "");
            croper.setImage("");
            $("#preview_large").removeAttr("style").attr("src", n.message);
            $("#crop_operation").hide();
            $("#crop_operation_msg").html("");
            $("#croped_message").html("裁切并保存成功");
            $("#preview_title").html("更新后的头像");
            //window.parent.location.reload(); //刷新父页面 if use iframe
        }
    })
});
    ...
    var n = function (n, t) {
        n.fineUploader({
            validation: {
                ...
                sizeLimit: 2097152 //2*1024*1024 //10485760
            },
            request: {
                endpoint: "/Avatar/ProcessUpload"
            },
            text: {
                uploadButton: '<i class="icon-upload icon-white"><\/i> 上传头像图片',
                dropProcessing: "（支持文件拖放上传，只能上传单张2M以下png、jpg图片）"
            },
